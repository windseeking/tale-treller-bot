...
import type {GeneratedTaskInput, GeneratedTaskOutput, TaskGenerator} from '#interfaces/task/task-generator.js'
import {env} from '#config/env.js'
import {AppError} from '#errors/app-error.js'
import llmOutputValidator from '#validators/llm/llmOutput.js'
import openAiResponseValidator from '#validators/llm/openAiResponse.js'
import appendBotSignature from '#utils/appendBotSignature.js'

export class LlmClient implements TaskGenerator {
    private readonly baseUrl = env.LLM_BASE_URL ?? 'https://api.openai.com/v1'
    private readonly systemPrompt = loadSystemPrompt()

    public async generateTask(params: GeneratedTaskInput): Promise<GeneratedTaskOutput> {
        ...
        const llmOutput = llmOutputValidator.validate(outputPayload.value)
        if (llmOutput.errors.length > 0) {
            throw new AppError({
                message: 'LLM output validation failed',
                code: 'LLM_OUTPUT_VALIDATION_ERROR',
                details: llmOutput.errors
            })
        }

        return {
            name: llmOutput.data.name,
            desc: appendBotSignature(llmOutput.data.desc),
...
function buildUserPrompt(params: { taskText: string; currentDate: string }): string {
    return [
...
function extractLlmContent(body: unknown): string {
    const parsedChatCompletions = openAiResponseValidator.validate(body)
    if (parsedChatCompletions.errors.length === 0) {
